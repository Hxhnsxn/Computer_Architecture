...
static inline vpn_t vaddr_vpn(vaddr_t addr) {
    ...
    return (vpn_t) (addr / PAGE_SIZE); // takes the values that are greater than the offset length
}
...
static inline uint16_t vaddr_offset(vaddr_t addr) {
    ...
    return (uint16_t) (addr % PAGE_SIZE); // takes the value that is less than or equal to the offset length
}
