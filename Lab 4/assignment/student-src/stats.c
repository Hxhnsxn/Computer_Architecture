...
*/
void compute_stats() {
    stats.accesses = stats.reads + stats.writes;    // Calcualte the total number of access base on the number of reads and writes
    stats.aat = (MEMORY_ACCESS_TIME * stats.accesses + DISK_PAGE_READ_TIME * stats.page_faults + DISK_PAGE_WRITE_TIME * stats.writebacks) / (double) stats.accesses;
}
